...
public class XSSFRichTextString implements RichTextString {
    ...
    public XSSFFont getFontOfFormattingRun(XSSFWorkbook book, int index) {
        if(index < 0 || index >= st.sizeOfRArray()) return null;
		...
		if (rpr != null) {
			...
			//ZSS-728
			//20140729, try to find the existing one
			Font fnt0 = book.getStylesSource().findFont(fnt.getBoldweight(), fnt.getXSSFColor(),
						fnt.getFontHeight(), fnt.getFontName(), fnt.getItalic(), 
						fnt.getStrikeout(), fnt.getTypeOffset(), fnt.getUnderline());
			
			if (fnt0 == null) {
				fnt.setThemesTable(getThemesTable());
				fnt.registerTo(book.getStylesSource()); // register as a new font(with new index)
				return fnt;
			} else {
