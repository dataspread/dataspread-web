...
public class XSSFPrintSetup implements PrintSetup {
    ...
    public void setScale(short scale) {
        if (scale < 10 || scale > 400) throw new POIXMLException("Scale value not accepted: you must choose a value between 10 and 400: "+scale);
    ...
    public short getFitWidth() {
        return (short) (pageSetup.isSetFitToWidth() ? pageSetup.getFitToWidth() : 0);
    }
    ...
    public short getFitHeight() {
        return (short) (pageSetup.isSetFitToHeight() ? pageSetup.getFitToHeight() : 0);
    }
    ...
    public int getCommentsMode() {
    	return getCellComment().getValue() - 1;    
    }
    @Override
    public void setCommentsMode(int mode) {
        pageSetup.setCellComments(STCellComments.Enum.forInt(mode + 1));
    }
    ...
    public int getErrorsMode() {
        STPrintError.Enum val = pageSetup.getErrors();
        return val.intValue() - 1;
    }
    @Override
    public void setErrorsMode(int mode) {
    	if (mode < 0 || mode > 3) mode = 0;
    	pageSetup.setErrors(STPrintError.Enum.forInt(mode + 1));
    }
}
