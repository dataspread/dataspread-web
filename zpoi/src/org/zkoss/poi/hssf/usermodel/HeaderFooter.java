...
//				}
				...
				_left = _left + text.substring(2, pos);
...
//				}
				...
				_center = _center + text.substring(2, pos);
				text = text.substring(pos);
				break;
			case 'R':
//				if (text.indexOf("&C") >= 0) {
...
//				}
				...
				_right = _right + text.substring(2, pos);
				text = text.substring(pos);
				break;
			default:
				// Mimics the behaviour of Excel, which would put it in the center.
				_center = _center + (pos > 2 ? text.substring(2) : "");
				break outer;
			}
		}
...
//			}
		...
		text = text.replaceAll("[^&]?\\&K[0-9A-Fa-f]{6,6}", ""); // &Krrggbb; rgb color
		text = text.replaceAll("[^&]?\\&K[0-9][0-9][+-][0-9][0-9]", ""); // &Kttsnn; theme color
