 ...
 */
@Internal
public class InternalSheet { //ZKPVT-78
     ...
     */
    private final WorksheetProtectionBlock _protectionBlock = new WorksheetProtectionBlock(this);
    ...
    //20140417, henrichen@zkoss.org: ZSS-576: enhanced protection
    private   FeatHdrRecord				_enhancedProtectionRecord  =    null;
    
    //20140807, henrichen@zkoss.org: PLV record
    private   PLVRecord					_plv = null;

    //20140917, henrichen@zkoss.org: PrintHeaders record
    ...
    private InternalSheet(RecordStream rs) {
        ...
        while (rs.hasNext()) {
            ...
            */
        	...
        	if (recSid == FeatHdrRecord.sid) {
        		if (((FeatHdrRecord)rec).isEnhancedProtection()) {
        			_enhancedProtectionRecord = (FeatHdrRecord) rec;
        		}
        		records.add(rec);
                continue;
            }
    ...
    protected InternalSheet() { //ZKPVT-78
        _mergedCellsTable = new MergedCellsTable();
        ...
        _psBlock.addLateHeaderFooter(createHeaderFooter()); //ZSS-746, 20140808, henrichen@zkoss.org: add headerfooter
        records.add(_psBlock);
        ...
        //ZSS-746
        _plv = createPLV();
        if (_plv != null) records.add(_plv);
        _selection = createSelection();
        records.add(_selection);
        records.add(_mergedCellsTable); // MCT comes after 'Sheet View Settings'
        //ZKPVT-78: check null
        final Record epr = getOrCreateEnhancedProtectionRecord();
        if (epr != null) records.add(epr); //ZSS-576: enhanced protection
    ...
    /* package */ static BOFRecord createBOF() {
        ...
        retval.setType(( short ) BOFRecord.TYPE_WORKSHEET);

        retval.setBuild(( short ) BOFRecord.BUILD);
        retval.setBuildYear(( short ) BOFRecord.BUILD_YEAR);
        retval.setHistoryBitMask(BOFRecord.HISTORY_MASK);
     ...
     */
    public DefaultRowHeightRecord createDefaultRowHeight() {
    ...
    public void setDefaultRowHeight(short dch) {
        ...
        defaultrowheight.setDefaultChanged(dch != DefaultRowHeightRecord.DEFAULT_ROW_HEIGHT);
    }
    ...
    private static DimensionsRecord createDimensions() {
        ...
        retval.setLastRow(0);             // one more than it is
        retval.setFirstRow(0);
        retval.setLastCol(( short ) 0);   // one more than it is
        return retval;
    }
...
//20101014, henrichen@zkoss.org: EscherAggregate could have been created and insert into records    	
    ...
    //20140417, henrichen@zkoss.org: zss-576 enhanced protection
    public FeatHdrRecord getOrCreateEnhancedProtectionRecord() {
    	if (_enhancedProtectionRecord == null) {
    		_enhancedProtectionRecord = new FeatHdrRecord(FeatHdrRecord.SHAREDFEATURES_ISFPROTECTION);
    	}
    	return _enhancedProtectionRecord;
    }

    //ZSS-746
    //20140807, henrichen@zkoss.org: add in PLV record
    /**
     * creates the PLV record and sets it to: <P>
     * wScalePLV = 0
     * fPageLayoutView = false
     * fRulerVisible = true
     * fWhitespaceHidden = false
     * @since 3.9.5
     */
    public PLVRecord createPLV() {
        PLVRecord retval = new PLVRecord();

        retval.setScalePLV(( short ) 0);
        retval.setPageLayoutView(false);
        retval.setRulerVisible(true);
        retval.setWhitespaceHidden(false);
        return retval;
    }
    
    //ZSS-746
    //20140808, henrichen@zkoss.org: add in HeaderFooter record
    /**
     * creates the HeaderFooter record.
     * @since 3.9.5
     */
    public HeaderFooterRecord createHeaderFooter() {
    	HeaderFooterRecord retval = new HeaderFooterRecord();
    	
    	retval.setScaleWithDoc(true);
    	retval.setAlignMargins(true);
    	return retval;
    }

    //ZSS-765
    //20140917, henrichen@zkoss.org: print headings
    /**
