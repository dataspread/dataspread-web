...
public final class ArrayPtg extends Ptg {
	...
	public String toFormulaString() {
		return toFormulaString(ZssContext.getCurrent().getLocale());
	}
	...
	public String toFormulaString(Locale locale) {
		...
		final char arrayComma = dot == ',' ? '\\' : ',';
		final char arraySemicolon = dot == ';' ? '\\' : ';';
		StringBuffer b = new StringBuffer();
		b.append("{");
	  	for (int y=0;y<getRowCount();y++) {
			if (y > 0) {
				b.append(arraySemicolon);
			}
			for (int x=0;x<getColumnCount();x++) {
		  		...
		  		b.append(getConstantText(o, locale));
		  	}
		}
		b.append("}");
		return b.toString();
	...
	public String toInternalFormulaString() {
		return toFormulaString(Locale.US);
	}

	//ZSS-565, 20140529, henrichen: apply locale
	private static String getConstantText(Object o, Locale locale) {
		...
		if (o instanceof Double) {
			return NumberToTextConverter.toText(((Double)o).doubleValue(), locale);
		}
