...
final class LookupUtils {
	...
	private static final class StringLookupComparer extends LookupValueComparerBase {
		...
		protected StringLookupComparer(StringEval se, boolean matchExact) {
			super(se);
			_value = se.getStringValue();
			if (matchExact) { 
				_pattern = Pattern.compile(toJavaPattern(_value), Pattern.CASE_INSENSITIVE);
			}
		}
		protected CompareResult compareSameType(ValueEval other) {
			...
			//ZSS-811
			if (_pattern != null) {
				Matcher m = _pattern.matcher(se.getStringValue());
				return CompareResult.valueOf(m.matches() ? 0 : 1);
			} 
		...
		//ZSS-811
		private String toJavaPattern(String value) {
			...
			for (int j = 0; j < len; ++j) {
				...
				if (escape == j) { // escape
					if (ch == '*' || ch == '?') { 
						// ~? -> \?
						// ~* -> \*
						sb.append('\\').append(ch); //change to use regex's escape character
					} else if (ch == '~') {
						// ~~ -> ~
						sb.append('~');
					} else {
						// *, ?, ~ are the only 3 legal character after ~
						// always return #NA
						return "";
					}
				} else { // non-escape
					...
					case '|':
					case '.':
						// escape regex's special characters
						sb.append('\\').append(ch);
						break;
					default:
						// normal case
			...
			// ends with '~'; illegal, always return #NA
			if (escape == len) return "";
			return sb.toString();
		}
	}
	...
	public static int lookupIndexOfValue(ValueEval lookupValue, ValueVector vector, boolean isRangeLookup) throws EvaluationException {
		LookupValueComparer lookupComparer = createLookupComparer(lookupValue, !isRangeLookup);
		int result;
		if(isRangeLookup) {
	...
	public static LookupValueComparer createLookupComparer(ValueEval lookupValue, boolean matchExact) {
		...
		if (lookupValue instanceof StringEval) {
			return new StringLookupComparer((StringEval) lookupValue, matchExact);
		}
