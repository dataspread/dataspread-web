...
public class TestDeepHopsSum implements AsyncTestcase {
    // TODO: fix overflow
    private SSheet _sheet;
    private int _N;
    private int answer;
    private int maxChainLength = 1000;
     ...
     *         ------------------------------------------------------
     *        |        v          v         v                       v
     * A1 -> B(1) -> B(N+1) -> B(N+2) -> B(N+3) -> ... -> B(N+maxChainLength)
     * |               |                   ^
     * |               ---------------------
     * |
     * ----> B(2)
    ...
    public TestDeepHopsSum(SSheet sheet, int N) {
        _sheet = sheet;
        _N = N;
        answer = 20;
        ...
        for (int i = N + 1; i < N + maxChainLength + 1; i++) {
            sheet.getCell(i, 1).setFormulaValue("B1 + SUM(B" + (N + 1) + ":B" + i + ")");
        }
